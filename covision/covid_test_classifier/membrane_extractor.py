...
from covision.segmentation.src_seg.transformations_seg import resize_image
from covision.segmentation.src_seg.utils_seg.miscellaneous import compute_bounding_box_coordinates


def extract_corrected_membrane(args, image, model, return_mask=False):
    """
    Extract the corrected membrane region from a single raw image
    @param args: argparse.Namespace object containing the configuration parameters
    @param image: np.ndarray containing the raw image or image path
    """
    if model.training:
        model.eval()
